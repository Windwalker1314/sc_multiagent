...
class DQATTEN(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.n_head = 4
        self.qkv_emb_dim = 32
        self.hypernet_emb = 16

        for i in range(self.n_head):
            wq = nn.Sequential(nn.Linear(self.args.state_shape, self.hypernet_emb),
                               nn.ReLU(),
                               nn.Linear(self.hypernet_emb, self.qkv_emb_dim, bias=False))
        ...
        self.w_head = nn.Sequential(nn.Linear(self.args.state_shape,self.hypernet_emb),
                                    nn.ReLU(),
                                    nn.Linear(self.hypernet_emb, self.n_head))
    ...
    def forward(self, z_values, states, obs):
        ...
        w, v = self.get_weights(q_vals, states, obs)
        w = w.reshape(-1, t, n)
        w = w.unsqueeze(3).expand(-1,-1,-1,nq) + 1e-10
        v = v.reshape(-1, t, 1)
        v = v.unsqueeze(3).expand(-1,-1,-1,nq)
        Z_weighted = w*z_values + v
    ...
    def get_weights(self, q_values, states, obs):
        ...
        for k, q, in zip(all_keys_out, all_querys_out):
            ...
            scaled_atten_out[q_values<=-999999] = -9999999
        ...
        w_head = w_head.view(-1, self.n_head, 1).repeat(1,1,n)
        atten_weights *= w_head
